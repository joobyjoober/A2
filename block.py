...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if self.children:
            for i in range(4):
                self.children[i]._update_children_positions(self._children_positions()[i])
        ...
        if self.smashable():
            ...
            for i in range(4):
                self.children.append(Block(self._children_positions()[i],
                                           self._child_size(),
                                           COLOUR_LIST[random.randint(0, len(COLOUR_LIST) - 1)],
                                           self.level + 1,
                                           self.max_depth))
    ...
    def combine(self) -> bool:
        ...
        for i in self.children:
            col.append(i.colour)
        colour = max(col, key=col.count)
        col.remove(colour)
        if len(col) == 3:
            return False
        if len(col) == 2:
            col.remove(max(col, key=col.count))
            if not col:
                return False
    ...
    def create_copy(self) -> Block:
        ...
        if not self.children:
            ...
            for nodes in self.children:
                b.children.append(nodes.create_copy())
            return b

    def _unit_copy(self) -> Block:
        b = Block(self.position,
                  self.size,
                  self.colour,
                  self.level,
        ...
        if self.level == self.max_depth:
            ...
            if not self.children:
                for i in range(4):
                    b.children.append(Block(b._children_positions()[i],
                                            b._child_size(),
                                            b.colour,
                                            b.level+1,
                                            b.max_depth))
            ...
            for i in self.children:
                b.children.append(i._unit_copy())
            return b
...
"""
    ...
    b2 = generate_board(5, 32)
    print("\n=== random board ===")
    print(b2)
    print("\n=== unit board ===")
    print(b2.create_copy()._unit_copy().__eq__(b2._unit_copy()))
